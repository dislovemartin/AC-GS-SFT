...
Ensure all assessments are evidence-based and include specific reasoning.`;
  ...
  private getFallbackResponse(request: AIAnalysisRequest, processingTime: number): AIAnalysisResponse {
    return {
      analysis: `Simulation-based analysis for LSU: "${request.lsu}". The QEC-SFT system has analyzed this requirement using fallback algorithms. This LSU involves ${request.analysisType} considerations that have been evaluated using local semantic processing capabilities.`,
      confidence: 0.6, // Moderate confidence for simulation
      recommendations: [
        'Review LSU for semantic clarity and consistency',
        'Consider breaking down complex requirements into smaller units',
        'Validate against established governance patterns',
        'Ensure compliance with relevant regulatory frameworks'
      ],
      riskFactors: [
        'External AI analysis service unavailable - using simulation mode',
        'Complex semantic structure may require expert review',
        'Limited context analysis without full AI capabilities'
      ],
      technicalDetails: {
        mode: 'simulation',
        fallback: true,
        originalRequest: request.analysisType,
        processingCapabilities: 'local semantic analysis'
      },
      processingTime
    };
  }
